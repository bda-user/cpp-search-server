...
private:
    ...
    QueryWord ParseQueryWord(string text) const {
        if (text.empty()) {
            throw invalid_argument("ParseQueryWord: word is empty");
        }
        bool is_minus = false;
        if (text[0] == '-') {
            is_minus = true;
        ...
        if (text.empty()){
            throw invalid_argument("ParseQueryWord: no symbol after -");
        }
        if(text[0] == '-') {
            throw invalid_argument("ParseQueryWord: word start --");
        }
