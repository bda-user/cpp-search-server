...
public:
    ...
    explicit SearchServer(const std::string_view stop_words_text)
        : SearchServer(static_cast<std::string>(stop_words_text)){}

    void AddDocument(int document_id, const std::string& document, DocumentStatus status, const std::vector<int>& ratings);

    void AddDocument(int document_id, const std::string_view document, DocumentStatus status, const std::vector<int>& ratings) {
        AddDocument(document_id, static_cast<std::string>(document), status, ratings);
    }

    void AddDocument(int document_id, const char* document, DocumentStatus status, const std::vector<int>& ratings) {
        AddDocument(document_id, static_cast<std::string>(document), status, ratings);
    }

    // policy: _ | seq | par, query: string | string_view, predicate
    ...
    std::vector<Document> FindTopDocuments(
        ...
        return FindTopDocuments(std::execution::seq,
                                static_cast<std::string>(raw_query), document_predicate);
    }
    ...
    // policy: _ | seq | par, query: string | string_view, status
    std::vector<Document> FindTopDocuments(
            const std::execution::sequenced_policy&,
    ...
    std::vector<Document> FindTopDocuments(
        ...
        return FindTopDocuments(std::execution::seq, static_cast<std::string>(raw_query),
            [status](int document_id, DocumentStatus document_status, int rating) {
                return document_status == status;
            });
    }
    ...
    std::vector<Document> FindTopDocuments(const std::string_view raw_query, DocumentStatus status) const {
        return FindTopDocuments(std::execution::seq, static_cast<std::string>(raw_query), status);
    }
    ...
    std::vector<Document> FindTopDocuments(
            const std::execution::parallel_policy&,
            const std::string_view& raw_query, DocumentStatus status) const {
        return FindTopDocuments(std::execution::par, static_cast<std::string>(raw_query),
            [status](int document_id, DocumentStatus document_status, int rating) {
                return document_status == status;
            });
    }

    // policy: _ | seq | par, query: string | string_view | char*
    std::vector<Document> FindTopDocuments(
            const std::execution::sequenced_policy&, const std::string& raw_query) const {
        return FindTopDocuments(std::execution::seq, raw_query, DocumentStatus::ACTUAL);
    }

    std::vector<Document> FindTopDocuments(
            const std::execution::sequenced_policy&, const std::string_view& raw_query) const {
        return FindTopDocuments(std::execution::seq, static_cast<std::string>(raw_query));
    }

    std::vector<Document> FindTopDocuments(
            const std::execution::sequenced_policy&, const char* raw_query) const {
        return FindTopDocuments(std::execution::seq, static_cast<std::string>(raw_query));
    }
    ...
    std::vector<Document> FindTopDocuments(const char* raw_query) const {
        return FindTopDocuments(std::execution::seq, static_cast<std::string>(raw_query));
    }

    std::vector<Document> FindTopDocuments(
            const std::execution::parallel_policy&, const std::string& raw_query) const {
        return FindTopDocuments(std::execution::par, raw_query, DocumentStatus::ACTUAL);
    }

    std::vector<Document> FindTopDocuments(
            const std::execution::parallel_policy&, const std::string_view& raw_query) const {
        return FindTopDocuments(std::execution::par, static_cast<std::string>(raw_query));
    }

    std::vector<Document> FindTopDocuments(
            const std::execution::parallel_policy&, const char* raw_query) const {
        return FindTopDocuments(std::execution::par, static_cast<std::string>(raw_query));
    }
    ...
    std::tuple<std::vector<std::string_view>, DocumentStatus> MatchDocument(
            const std::string& raw_query, int document_id) const; //CPP

    std::tuple<std::vector<std::string_view>, DocumentStatus> MatchDocument(
            const std::string_view raw_query, int document_id) const {
        return MatchDocument(static_cast<std::string>(raw_query), document_id);
    }
    ...
    std::tuple<std::vector<std::string_view>, DocumentStatus> MatchDocument(
            const std::execution::parallel_policy& policy,
            const std::string_view raw_query, int document_id) const {
        return MatchDocument(policy, static_cast<std::string>(raw_query), document_id);
    }
...
private:
    ...
    QueryVec ParseQueryVec(const std::string& text) const;
...
std::vector<Document> SearchServer::FindTopDocuments(
    ...
    sort(std::execution::par_unseq, matched_documents.begin(), matched_documents.end(), [](const Document& lhs, const Document& rhs) {
        if (std::abs(lhs.relevance - rhs.relevance) < MAX_DELTA_RELEVANCE) {
            return lhs.rating > rhs.rating;
        } else {
            return lhs.relevance > rhs.relevance;
